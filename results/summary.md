...
**NWP 수집 완주**: 65,808/65,808 호출, 실패 0, 20.9GiB — 2021-07~2023-12 제주 13개 지점, 리드 매칭 밴드 2종 (26–49h → h≤24 / 50–73h → h≤48)
...
- **주 (2026-07-22 명시화)**: 위 표는 G3 시점의 **초기 탐색 프로토콜**(results/lps.csv) 수치다.
  사전 등록에 쓰인 분류는 같은 날 확정된 **공식 프로토콜**(per-channel mean + expanding CV,
  `results/lps_official.csv` — 사전 등록 커밋 `cab17c1`에 포함)이며, 공식값은 etth2 = **−0.205**,
  electricity = **0.283**으로 둘 다 τ=0.3 아래(→ "−" 예측). 위 ⚠️ 경고(0.03~0.93 불안정)의 실체가
  바로 이 프로토콜 민감도이고, 공식 프로토콜 확정·커밋은 **모든 grid 실행 이전**이다
  (해당 커밋 메시지: "resolves etth2 OT-channel anomaly: 0.89 → −0.21").
