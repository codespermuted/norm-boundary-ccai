...
- covfair에서 백본 유연성↑·공변량 접근↑일수록 CN의 한계효용 감소: linmix(RAW−CN +0.127) → mlpmix(+0.052) → lgbmcov(−0.009).
...
- 품질 관리: 초안을 6개 검증 에이전트(수치/수학/구조/인용/과대주장/PDF육안)로 공격 → 25건 수정.
  대표: 블록 B "전 백본 최적" 과장 정정, 낡은 covfair 격차(+0.135/+0.064)를 +0.127/+0.052/−0.009로
...
- **재현 성공**: Tab 1 40셀·GATE 2 격차·분산 귀속·ρ·τ 민감도·ΔLPS 전부 원천 CSV에서 인쇄 정밀도로 재현.
  별표 32개·굵게 8개 DM 재계산 일치, MCS 23셀 생존 집합 일치(부트스트랩 시드 1–3 불변), 오차 배열 길이=세그먼트
  인지 윈도우 수·평균=CSV mse. 사전등록: cab17c1(13:20:23) < 최초 run(13:24:36), filter-branch 날짜 보존 확인.
- **정정 1건 (블록 B mlpmix)**: gefcom_solar×mlpmix 75키가 CSV에 이중 기록(재실행 값 상이). npy와 일치하는
  재실행을 canonical로 확정, 구실행 75행 제거(1,293→1,133), tabB·논문 갱신: mlpmix 행 0.1963/0.2201/0.2402/
  0.4057/0.1443, RAW−CN 격차 +0.053→**+0.052**. 순위·DM 카운트·정성 결론 전부 불변.
- 조판 2건 수정(부록 표 B.5 클리핑, 마지막 쪽 캡션-페이지번호 충돌), ridge 민감도(6/8) 본문 명시,
  history-rewrite 투명성 문장 app:repro 추가.
