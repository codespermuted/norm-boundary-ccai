...
## 5.1 괴리 기록 (G2에서 발견, 2026-07-13)

**가정 A4의 CN 대칭 위반 — 선형 백본의 암묵적 수준 추적.** G2 부분 데이터에서 실험 λ*(≈0.01–0.03)가
M1 예측(≈0.33)보다 훨씬 작게 관측됨. 원인: 선형 백본은 입력 윈도우(잔차)에서 남은 수준 드리프트를
OLS적으로 추적하므로, 조작적 CN(그리고 RAW)은 M1의 "순수 복원 규칙"보다 강하다. M1의 λ*는 **상한**.

**대응 (명제 2′)**: 각 정규화의 함수 클래스에서 **제약 OLS의 닫힌형 최적 MSE**를 정확한 이론값으로
사용 (`src/theory/linear_class.py`, Toner & Darlow 기계 — 계획서 §2.2의 증명 전략과 동일 도구).
GATE 1의 λ* 판정은 OLS 이론 기준. M1은 해석층(왜 교차가 생기는가)으로 유지하고, λ*_M1 − λ*_OLS
격차 자체를 "암묵적 수준 추적의 크기"로 논문에 보고 (Discussion 소재).

**논문 함의**: σ_Δ=0(공변량 직교 이동 없음)에서 CN은 아주 작은 λ부터 우세 — IN의 실질 영역은
드리프트 강건성이 필요한 영역으로 좁혀짐. 이는 주장을 약화시키지 않고 오히려 실무 메시지를 강화.
