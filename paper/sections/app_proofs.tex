...
\emph{Step 1: the RevIN-restored class lies in $\mathcal{F}$.}
A RevIN-type predictor with an affine backbone computes $\hat y = f(x - \bar y \mathbf{1}) + \bar y$, where $f$ is affine and the normalized features are level-free \citep{kim2022revin}. When the instance information is summarized by the pair $(\bar y, g)$, the normalized features are affine in $g$ alone, hence $\hat y = \alpha_0 + \bar y + \alpha_2 g$: an element of $\mathcal{F}$ with the $\bar y$-coefficient pinned to one. The lower bound below for $\mathcal{F}$ therefore applies \emph{a fortiori} to the RevIN class.
