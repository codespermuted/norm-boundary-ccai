...
Proposition~1 is a statement about linear function classes. To probe
whether its geometry survives nonlinearity, we train an identical
...
below a provisional class-specific floor
$\kappa^2\,\var(\bar y \mid g)\,\var(g)$ (its mean sits above the floor by
at least $16$ standard errors of the mean at both $\lambda$), while the
unconstrained and covariate-restored arms sit roughly a factor of twenty
below it. The floor
used here is the conditional-variance analogue of the linear-class gap of
Proposition~1 (a nonlinear class can represent covariate nonlinearities,
so its irreducible excess is smaller); its constant is deliberately kept
configurable pending a formal derivation, and the demonstration is
reported as measurement, not proof.
