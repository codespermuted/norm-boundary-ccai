...
exact code state that produced it. The logged metrics are per-run scalars, not
per-epoch trajectories; the CondNorm arm's logged \texttt{val\_mse} is recorded
on a residual-standardized rather than global-z scale --- a benign logging
artifact that leaves every reported \texttt{test\_mse} unchanged, analysed with
its impact in \texttt{evidence/condnorm\_val\_scale\_diagnosis.md}.
