...
\begin{tabular}{@{}lp{2.8cm}llp{3.1cm}@{}}
\toprule
...
PatchTST \citep{nie2023patchtst} & RevIN, on by default & removed; external arm & \texttt{revin} & none (A) / PatchTSTCov (B) \\
SegRNN \citep{lin2023segrnn} & instance norm (last-value subtraction) & removed; external arm & $\approx$\,\texttt{revin} (item~1) & none (A) / SegRNNCov (B) \\
...
mlpmix $+0.053$, LGBM+cov $-0.009$ --- consistent with a flexible backbone
learning the restoration map internally when given the covariates. Crucially,
the RevIN gap does \emph{not} shrink: RevIN+cov is worse than \rawm{}+cov for
...
mlpmix       & 0.1978 & 0.2214 & 0.2419 & 0.4081 & \textbf{0.1446} & 10 \\
...
to the forecast irradiance regime.

\begin{figure}[t]
\centering
\includegraphics[keepaspectratio,width=\textwidth,height=0.85\textheight]{figures/sample_forecasts.png}
...
forecasts. On the wind rows (jeju\_wind, gefcom\_wind), RevIN's restoration is
anchored to lookback-window statistics and lags or misses ramps, while
CondNorm follows the NWP-implied level; on the gefcom\_solar row, the
overcast-day window shows RevIN restoring the level of the preceding sunny
window while CondNorm adjusts it down to the forecast irradiance regime. The
standard-benchmark rows (etth1, etth2, weather) show CondNorm's failure mode
outside the applicability boundary: its covariate-conditional restoration
places the forecast at a visibly wrong level.}
\label{fig:samples}
\end{figure}
